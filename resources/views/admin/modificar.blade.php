...
<script>
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      fetch(`/api/usuarios2/eliminar/${documentId}`, {
        method: "DELETE"
      })
        .then((response) => response.json())
        .then((data) => {
          if (data.mensaje === "Usuario eliminado exitosamente") {
            ...
            document.getElementById('userCount').textContent = data.usuarios.length;

            Swal.fire("Eliminado", "El usuario ha sido eliminado correctamente.", "success").then(() => {
              // Puedes realizar alguna acción adicional después de eliminar el usuario
              console.log(data);
            });
          } else {
